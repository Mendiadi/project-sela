...
class ResultPage(BasePage):
    ...
    def product_list(self) -> [WebElement]:
        return self.driver.locate_elements(self.locators['products'])
    ...
    def find_cheapest_product(self) -> [WebElement]:
        ...
        for product in products:
            link = self.driver.locate_element(self.locators['product_link'], product)
            right_block = self.driver.locate_element(self.locators['right_block'], product)
            try:
                prices = self.driver.locate_element(self.locators['price'], right_block)
                price = self.driver.locate_element(self.locators["price_span"], prices)
            except Exception:
                price = self.driver.locate_element(self.locators['price'], right_block)
    ...
    def add_product_to_cart(self, product: WebElement):
        product.click()
        self.driver.locate_and_switch_to_frame(self.locators['iframe_product'])
        self.driver.locate_element(self.locators['add_to_card_btn']).click()

    def process_to_checkout(self) -> CheckOutPage:
        layer = self.driver.locate_element(self.locators['checkout_layer'])
        btn_container = self.driver.locate_element(self.locators['checkout_container'], layer)
        self.driver.locate_element(self.locators['checkout_btn'], btn_container).click()
        return CheckOutPage(self.driver)

    def continue_shopping(self):
        layer = self.driver.locate_element(self.locators['checkout_layer'])
        btn_container = self.driver.locate_element(self.locators['checkout_container'], layer)
        self.driver.locate_element(self.locators['continue_shop'], btn_container).click()
